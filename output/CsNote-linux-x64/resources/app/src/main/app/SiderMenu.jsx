    ...
    data.map((item, index) => {
      ...
      if (item.key === this.state.currentHandleKey) {
        ...
        if (undefined !== item.children && 0 !== item.children.length)
          return Modal.warning({ title: '提示', content: '请将该目录下的笔记本删除!', centered: true, closable: false, okText: '好的', width: 400, });
        // 判断叶子下面是否含有文章
        let tag = true;
        ...
        if (!tag)
          return Modal.warning({ title: '提示', content: '请将该笔记本下的文章删除!', centered: true, closable: false, okText: '好的', width: 400, });
        // 删除此节点下的所有分支
  ...
  render() {
    ...
    return (
      <div className="siderMenu" style={{height: height, overflow: 'auto'}}>
        <div style={{height: '32px', margin: '8px', fontSize: '24px'}} >目录</div>
