...
import { Radio, Input, Tree, Icon, Modal } from 'antd';
...
class SiderMenu extends Component {
  ...
  loopToRender = data => data.map((item) => {
    ...
    // 拼接标题,搜索的关键字用红字标出
    let title = index > -1 ? <span>{beforeString}<span style={{color: '#f50', fontWeight: 'bold'}}>{this.state.searchValue}</span>{afterString}</span> : <span>{item.title}</span>;
    // 如果树下有子节点,则进行递归遍历
    if (item.children)
  ...
  render() {
    ...
    return (
      <div className="siderMenu" style={{height: height, overflow: 'auto', WebkitAppRegion: 'no-drag'}}>
        <div style={{height: '48px', padding: '8px', fontSize: '24px', color: '#fff', WebkitAppRegion: 'drag'}} >库</div>
        <div style={{position: 'fixed', top: '16px', left: '56.5px', color: '#fff', WebkitAppRegion: 'no-drag'}}>
          <Radio.Group value={this.state.store} buttonStyle="solid" size="small" onChange={(event) => this.setState({store: event.target.value})}>
            <Radio.Button value="local">本地</Radio.Button>
            <Radio.Button value="cloud">云端</Radio.Button>
          </Radio.Group>
        </div>
