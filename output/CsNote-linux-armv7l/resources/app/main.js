...
ipcMain.on('MENU', (event, arg) => {
  ...
  if ('MENU_SIDERMENU' === arg.action) {
    ...
    menu.append(new MenuItem({ label: '编辑', click: () => {
      event.sender.send('MENU_OUTLINE', {action: 'MENU_OUTLINE_EDIT_ARTICAL', key: arg.key});
    } }));
    menu.append(new MenuItem({ label: '重命名', click: () => {
      event.sender.send('MENU_OUTLINE', {action: 'MENU_OUTLINE_RENAME_ARTICAL', key: arg.key});
    } }));
    menu.append(new MenuItem({ type: 'separator' }));
    ...
    menu.append(new MenuItem({ label: '粘贴文本(只有文字)', click: () => {
    ...
    menu.append(new MenuItem({ label: '粘贴截图(适用于截图,ppt)', click: () => {
      // 判断是否含有图片
