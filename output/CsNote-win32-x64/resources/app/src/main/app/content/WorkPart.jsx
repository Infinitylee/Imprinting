...
class WorkPart extends Component {
  ...
  componentWillMount() {
    ...
    ipcRenderer.on(Ipc.FILE, (event, arg) => {
      if (Ipc.FILE_READ === arg.action && Ipc.FILE_READ_WORKPART === arg.callback) {
        ...
        // 设置数据
        this.setState({defaultArticalData: arg.data.data, currentArticalData: arg.data.data});
      }
    });
    // 注册右键处理事件
  ...
  onUpdateArticalData = (key) => {
    if (undefined !== key)
      ...
      this.setState({defaultArticalData: ''});
  }
  ...
  onQuillChange(content, delta, source, editor) {
    if (undefined !== this.props.selectOutline) {
      ...
      this.setState({defaultArticalData: '', currentArticalData: ''});
    }
  }
  ...
  render() {
    const height = this.props.height - 32 - 32 - 58;
    return (
      <div style={{ width: '100%', height: '100%', overflowY: 'auto', background: '#f1f1f1' }}>
        <Row type="flex" justify="space-between" align="middle" style={{height: '48px', fontSize: '24px', borderLeft: '1px solid #ccc', borderRight: '1px solid #ccc'}}>
          <Col span={20} style={{padding: '8px'}}>{undefined !== this.props.selectOutlineTitle ? this.props.selectOutlineTitle : '标题'}</Col>
          <Col span={4} style={{padding: '8px', textAlign: 'right'}}>
            <Button type="primary" shape="circle" icon="check" style={{marginBottom: '6px'}} onClick={() => this.onSaveClickHandle()}/>
          </Col>
        </Row>
