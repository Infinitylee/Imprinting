...
class Outline extends Component {
  ...
  componentWillMount() {
    ...
    ipcRenderer.on(Ipc.MENU_OUTLINE, (event, arg) => {
      if (Ipc.MENU_OUTLINE_EDIT_ARTICAL === arg.action) { // 编辑文章
        // 打开文章
        this.onArticalClick(arg.key);
      } else if (Ipc.MENU_OUTLINE_RENAME_ARTICAL === arg.action) { // 重命名文章标题
        // 获取文章列表
        let articalList = this.getArtical(this.props.outlineData, this.props.selectSiderMenu);
        // 初始化值
  ...
  onCreateNewArticalHandle = () => {
    if (undefined !== this.props.selectSiderMenu) {
      // 初始化数据,显示模态框
      this.setState({modalVisible: true, modalInputText: '', modalAction: 'add'});
    } else {
      // 节点未选择警告
      Modal.warning({ title: '提示', content: '请选择节点!', centered: true, closable: false, okText: '好的', width: 400, });
    }
  }
    ...
    confirm({
      ...
      onOk() {
        ...
        if (that.state.currentHandleKey === that.props.selectOutline) {
          ...
          that.props.setSelectOutlineTitle('');
        }
        // 同步记录到本地
        ipcRenderer.send(Ipc.FILE, {action: Ipc.FILE_WRITE, dir: '/data/outLine.cs', data: JSON.stringify(tempOutlineData)});
        // 删除文件
        ...
        // 更新新数据,关闭对话框,清空临时数据
        that.setState({ currentHandleKey: '' }, () => {
   ...
   */
  onRightClick = (key) => {
    ipcRenderer.send(Ipc.MENU, {action: Ipc.MENU_OUTLINE, key: key});
  }
  ...
  modalConfirmHandle = (event) => {
    ...
    if ('add' === this.state.modalAction) { // 新建文章
      ...
      for (let i = 0; i < tempOutlineData.length; i++) {
        if (this.props.selectSiderMenu === tempOutlineData[i].key) {
          ...
          tempTag = true;
          // 添加对象
          tempOutlineData[i]['artical'].push({key: articalKey, title: this.state.modalInputText});
          break;
        }
      }
      // 标记处理
      if (!tempTag) {
        // 新建对象
        let newIndex = {key: this.props.selectSiderMenu, artical: [{key: articalKey, title: this.state.modalInputText}]};
        // 添加对象
      ...
      for (let i = 0; i < tempOutlineData.length; i++) {
        if (this.props.selectSiderMenu === tempOutlineData[i].key) {
          ...
          tempOutlineData[i]['artical'].map((item) => {
            return item.key === this.state.currentHandleKey ? item.title = this.state.modalInputText : '';
          });
          break;
        }
      }
    ...
    // 更新新数据,关闭对话框,清空临时数据
    this.props.setSelectOutlineTitle(this.state.modalInputText);
    this.props.setOutlineData(tempOutlineData);
    this.setState({modalVisible: false, modalInputText: ''}, () => {
      // 更新数据后同步记录到本地
      ipcRenderer.send(Ipc.FILE, {action: Ipc.FILE_WRITE, dir: '/data/outLine.cs', data: JSON.stringify(tempOutlineData)});
      // 创建新文件
      if ('add' === this.state.modalAction) {
        ipcRenderer.send(Ipc.FILE, {action: Ipc.FILE_WRITE, dir: '/data/artical/' + articalKey + '.cs', data: '{"data":""}'});
      }
    });
  }
  ...
  render() {
    return (
      <div style={{ width: '100%', height: '100%', overflowY: 'auto' }}>
        <Row type="flex" justify="space-between" align="middle" style={{height: '32px', margin: '8px', fontSize: '24px'}}>
          <Col span={6}>大纲</Col>
          <Col span={4} style={{textAlign: 'right'}}>
            <Button type="primary" shape="circle" icon="plus" style={{marginBottom: '6px'}} onClick={() => this.onCreateNewArticalHandle()}/>
          </Col>
        </Row>
        <List
              ...
              <List.Item.Meta
                title={
                  <a
                    ...
                    style={{
                      textDecoration: 'none',
                      color: this.props.selectOutline === item.key ? '#1890ff' : 'rgba(0, 0, 0, 0.65)',
                      // fontWeight: this.props.selectOutline === item.key ? 'bold' : 'normal',
                    }}
                    onClick={() => this.onArticalClick(item.key, item.title)}
                    onContextMenu={() => this.onRightClick(item.key)}
                  >
                    {item.title}
                  </a>
                }
                description={
                  <span style={{display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: '3', overflow: 'hidden'}}>
                    {undefined !== item.preview ? item.preview : '暂无内容'}
                  </span>
                }
              />
            </List.Item>
